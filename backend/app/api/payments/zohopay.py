...
class ZohoOAuthExchangeRequest(BaseModel):
    ...
    client_secret: str
    redirect_uri: str
    account_id: Optional[str] = None  # ZohoPay Account ID
    api_key: Optional[str] = None  # ZohoPay API Key (if different from access_token)
    ...
    try:
        ...
        try:
            ...
            if tokens and len(tokens) > 0:
                ...
                print(f"[get_zohopay_config] Token keys: {list(token.keys())}")
                print(f"[get_zohopay_config] account_id: {token.get('account_id')}")
                print(f"[get_zohopay_config] api_key: {token.get('api_key')}")
                # Return all necessary fields including account_id and api_key
                config = {
                    "access_token": token.get("access_token"),
                    "api_key": token.get("api_key") or token.get("access_token"),  # Use api_key if available, fallback to access_token
                    "api_domain": token.get("api_domain"),
                    "domain": "IN",  # Default domain for ZohoPay
                }
                # Add account_id if available
                if token.get("account_id"):
                    config["account_id"] = token.get("account_id")
                # Add refresh_token if available
                if token.get("refresh_token"):
                    config["refresh_token"] = token.get("refresh_token")
                print(f"[get_zohopay_config] Returning config with keys: {list(config.keys())}")
                return config
...
async def exchange_oauth_token(request: ZohoOAuthExchangeRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        if db_service.is_available():
            try:
                # Use api_key from request if provided, otherwise use access_token
                api_key = request.api_key or access_token
                ...
                token_data = {
                    ...
                    "account_id": request.account_id or "",  # Store account_id if provided
                    "api_key": api_key,  # Store api_key (use provided or access_token)
    ...
    try:
        ...
        if db_service.is_available():
            try:
                ...
                # Get existing token to preserve account_id if not provided in request
                existing_tokens = db_service.admin_get_all("zoho_oauth_tokens", limit=1)
                existing_account_id = existing_tokens[0].get("account_id") if existing_tokens else None
                ...
                token_data = {
                    ...
                    "token_type": result.get("token_type", "Bearer"),
                    "scope": result.get("scope", ""),
                    "account_id": request.account_id or existing_account_id or "",  # Preserve existing or use new
                    "api_key": api_key,  # Store api_key (use provided or access_token)
...
async def get_zohopay_widget_config() -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"[ZohoPay Config] account_id value: {account_id[:20] if account_id else 'None'}...")
        print(f"[ZohoPay Config] api_key value: {api_key[:20] if api_key else 'None'}...")
    ...
    try:
        ...
        try:
            ...
            debug_info["zoho_oauth_tokens"] = {
                ...
                "data": tokens[:2] if tokens else None,  # Limit to first 2 for debugging
            }
            
            if tokens and len(tokens) > 0:
                token = tokens[0]
                debug_info["zoho_oauth_tokens"]["keys"] = list(token.keys())
                debug_info["zoho_oauth_tokens"]["has_account_id"] = bool(token.get("account_id"))
                debug_info["zoho_oauth_tokens"]["has_api_key"] = bool(token.get("api_key"))
                debug_info["zoho_oauth_tokens"]["has_access_token"] = bool(token.get("access_token"))
                debug_info["zoho_oauth_tokens"]["account_id_preview"] = token.get("account_id")[:20] + "..." if token.get("account_id") else None
                debug_info["zoho_oauth_tokens"]["api_key_preview"] = token.get("api_key")[:20] + "..." if token.get("api_key") else None
        except Exception as e:

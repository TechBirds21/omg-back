...
class StoreBillCreate(BaseModel):
    ...
    phonepe_order_id: Optional[str] = None  # PhonePe order ID if payment via PhonePe
...
async def create_store_bill(bill_data: StoreBillCreate) -> Dict[str, Any]:
    ...
    try:
        ...
        # Payment method must be one of: cash, card, upi, phonepe, other
        payment_method = (bill_data.payment_method or "cash").lower()
        if payment_method not in ["cash", "card", "upi", "phonepe", "other"]:
        ...
        # Combine notes with PhonePe order ID if provided
        notes_parts = []
        if bill_data.notes:
            notes_parts.append(bill_data.notes)
        if bill_data.phonepe_order_id:
            notes_parts.append(f"PhonePe Order ID: {bill_data.phonepe_order_id}")
        if notes_parts:
            bill_data_dict["notes"] = "\n".join(notes_parts)

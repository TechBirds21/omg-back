  ...
  useEffect(() => {
    const loadCollectionsWithProducts = async () => {
      try {
        ...
        // Show all active categories (not just first 3) - so dresses, pattu, etc. all display
        const activeCategories = categories.filter(cat => cat.is_active !== false);

        const collectionsData = await Promise.all(
          activeCategories.map(async (category) => {
            const products = await fetchProducts({
              categoryId: category.id, // Use category ID (UUID) instead of name
              limit: 8 // Increased from 4 to show more products per category
            });
            return { category, products };
          })
        );

        // Only filter out categories with zero products, keep all others
